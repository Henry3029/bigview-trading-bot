...
exports.createInitialPositionState = createInitialPositionState;
exports.processActivePosition = processActivePosition;
 ...
 */
function emitEngineLog(io, engineName, type, message) {
    if (!io)
        return;
    io.emit('engine_log', {
...
async function processActivePosition(exchange, position, currentPrice, io) {
    const { activeAsset, takeProfitPrice, stopLossPrice, tradeAmountUnits, entryTime, entryPrice, hasTakenPartialProfit = false, highestPriceSinceEntry = 0, tierTargetLocked = false, lockedProfitPct = 0 } = position;
    ...
    const STAGNANT_TIMEOUT_MS = config_1.CONFIG.STAGNANT_TIMEOUT_MS || (24 * 60 * 60 * 1000); // 24 Hours
    // Terminal Logging
    console.log(`[TRADE ACTIVE: ${cleanAsset}] Price: $${currentPrice} | Peak: $${currentHighestPrice.toFixed(4)} (+${peakPriceChangePct.toFixed(2)}%) | ` +
    ...
    if (elapsedTimeMs >= STAGNANT_TIMEOUT_MS && peakPriceChangePct < 0.20) {
        ...
        try {
            await (0, distributionService_1.processTradeProfitDistribution)({
 ...
 */
function createExecutionRecord(mode, asset, action, executionPrice, status) {
    return {
        mode,
        asset,
        action,
 ...
 */
async function executeSell(exchange, asset, rawUnits, executionPrice, reason) {
    const mode = config_1.CONFIG.DRY_RUN ? 'DRY_RUN' : 'LIVE';
    if (config_1.CONFIG.DRY_RUN) {
        console.log(`🧪 [DRY_RUN] Simulated sell of ${rawUnits} units of ${asset}. Reason: ${reason}`);
        (0, logger_1.logAIDecision)(reason, `Simulated exit for ${asset} (${rawUnits} units)`, createExecutionRecord(mode, asset, 'SELL', executionPrice, 'SIMULATED_SUCCESS'));
        return true;
    }
    try {
        ...
        (0, logger_1.logAIDecision)(reason, `Executed sell for ${asset} (${validUnits} units)`, createExecutionRecord(mode, asset, 'SELL', executionPrice, 'FILLED'));
        return true;
    }
    catch (exitError) {
        console.error(`❌ Critical: Failed to execute sell order on ${asset}: ${exitError.message}`);
        (0, logger_1.logAIDecision)('SELL_ORDER_FAILED', `Failed sell attempt on ${asset}: ${exitError.message}`, createExecutionRecord(mode, asset, 'SELL', executionPrice, 'FAILED'));
        return false;
    }
}
